...
| 19    | User's credentials, payment card details, etc| Phishing attacks leveraging malicious JS on landing page | 1) Landing page URL fragement contains encryption key - meaning malicious JS could use key to decrypt contents, 2) Malicious JS can phish for user credentials, payment card information, or other sensitive data | 1) Properly vet Java Script that is embedded on landing page, 2) Define strong content security policy |   |
